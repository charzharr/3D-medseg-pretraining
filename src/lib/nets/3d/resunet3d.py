...
def create_conv(in_channels, out_channels, kernel_size, order, 
                num_groups, padding):
    """
    ...
    msg = 'Non-linearity cannot be the first operation in the layer'
    assert order[0] not in 'rle', msg
    ...
    for i, char in enumerate(order):
        if char == 'r':
            ...
            msg = (f'Expected number of channels in input to be divisible by '
            f'num_groups. num_channels={num_channels}, num_groups={num_groups}')
            assert num_channels % num_groups == 0, msg
            ...
            msg = (f"Unsupported layer type '{char}'. MUST be one of "
                  f"['b', 'g', 'r', 'l', 'e', 'c']")
...
def create_encoders(in_channels, f_maps, basic_module, conv_kernel_size, 
                    conv_padding, layer_order, num_groups, pool_kernel_size):
    # create encoder path consisting of Encoder modules. Depth of the encoder 
    # is equal to `len(f_maps)`
    encoders = []
    for i, out_feature_num in enumerate(f_maps):
        if i == 0:
            encoder = Encoder(in_channels, out_feature_num,
                              apply_pooling=False,  # skip pooling in 1st enc
            ...
            # TODO: adapt for anisotropy in the data, i.e. use proper pooling 
            # kernel to make the data isotropic after 1-2 pooling operations
...
def create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding, 
    ...
    # create decoder path consisting of the Decoder modules. The length of the 
    ...
    for i in range(len(reversed_f_maps) - 1):
        ...
        _upsample = True
        if i == 0:
            # upsampling can be skipped only for the 1st decoder, afterwards 
...
class SingleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, order='gcr', 
...
class DoubleConv(nn.Sequential):
    ...
    same as (H_in, W_in), so that you don't have to crop in the decoder path.
    Args:
        in_channels (int): number of input channels
        out_channels (int): number of output channels
        encoder (bool): if True we're in the encoder path, otherwise we're 
            in the decoder
        kernel_size (int or tuple): size of the convolving kernel
        order (string): determines the order of layers, e.g.
            'cr' -> conv + ReLU
    ...
    def __init__(self, in_channels, out_channels, encoder, kernel_size=3, 
        ...
        if encoder:
            ...
            # we're in the decoder path, decrease the number of channels in the 
            # 1st convolution
...
class ExtResNetBlock(nn.Module):
    ...
    Notice we use ELU instead of ReLU (order='cge') and put non-linearity after 
    the groupnorm.
    """

    def __init__(self, in_channels, out_channels, kernel_size=3, order='cge', 
                 num_groups=8, **kwargs):
        super(ExtResNetBlock, self).__init__()

        # first convolution
        self.conv1 = SingleConv(in_channels, out_channels, 
                                kernel_size=kernel_size, order=order, 
                                num_groups=num_groups)
        # residual block
        self.conv2 = SingleConv(out_channels, out_channels, 
                                kernel_size=kernel_size, order=order, 
                                num_groups=num_groups)
        # remove non-linearity from the 3rd convolution since it's going to be 
        # applied after adding the residual
        n_order = order
        for c in 'rel':
            n_order = n_order.replace(c, '')
        self.conv3 = SingleConv(out_channels, out_channels, 
                                kernel_size=kernel_size, order=n_order,
                                num_groups=num_groups)
...
class TransposeConvUpsampling(AbstractUpsampling):
    ...
    def __init__(self, in_channels=None, out_channels=None, kernel_size=3, 
        ...
        upsample = nn.ConvTranspose3d(in_channels, out_channels, 
...
class Abstract3DUNet(BaseModel):
    ...
    Args:
        ...
        out_channels (int): number of output segmentation masks;
            ...
            use the proper loss criterion during training (i.e. 
            CrossEntropyLoss (multi-class) or BCEWithLogitsLoss 
            (two-class) respectively)
        f_maps (int, tuple): number of feature maps at each level of the 
            encoder; if it's an integer the number of feature maps is given by 
        ...
        layer_order (string): determines the order of layers
            in `SingleConv` module. e.g. 'crg' stands for 
                Conv3d+ReLU+GroupNorm3d.
            See `SingleConv` for more info
        num_groups (int): number of groups for the GroupNorm
        num_levels (int): number of levels in the encoder/decoder path 
            (applied only if f_maps is an int)
        is_segmentation (bool): if True (semantic segmentation problem) 
            Sigmoid/Softmax normalization is applied
            after the final convolution; if False (regression problem) the 
            normalization layer is skipped at the end
        testing (bool): if True (testing mode) the `final_activation` 
            (if present, i.e. `is_segmentation=true`)
            will be applied as the last operation during the forward pass; if 
            False the model is in training mode
            and the `final_activation` (even if present) won't be applied; 
            default: False
        conv_kernel_size (int or tuple): size of the convolving kernel in the 
            basic_module
        pool_kernel_size (int or tuple): the size of the window
        conv_padding (int or tuple): add zero-padding added to all three 
            sides of the input
    """

    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, 
                 f_maps=64, layer_order='gcr', num_groups=8, num_levels=4, 
                 is_segmentation=True, testing=False, conv_kernel_size=3, 
                 pool_kernel_size=2, conv_padding=1, **kwargs):
        ...
        # create decoder path
        self.decoders = create_decoders(f_maps, basic_module, conv_kernel_size, 
    ...
    def forward(self, x):
        ...
        # apply final_activation (i.e. Sigmoid or Softmax) only during 
        # prediction. During training the network outputs
        # logits and it's up to the user to normalize it before visualising 
        # with tensorboard or computing validation metric
...
class UNet3D(Abstract3DUNet):
    ...
    Uses `DoubleConv` as a basic_module and nearest neighbor upsampling in the 
        decoder
    """

    def __init__(self, in_channels, out_channels, final_sigmoid=True, 
                 f_maps=64, layer_order='gcr', num_groups=8, num_levels=4, 
                 is_segmentation=True, conv_padding=1, **kwargs):
...
class ResidualUNet3D(Abstract3DUNet):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid=True, 
                 f_maps=64, layer_order='gcr', num_groups=8, num_levels=5, 
                 is_segmentation=True, conv_padding=1, **kwargs):
...
class UNet2D(Abstract3DUNet):
    """
    Just a standard 2D Unet. Arises naturally by specifying 
    conv_kernel_size=(1, 3, 3), pool_kernel_size=(1, 2, 2).
    """
