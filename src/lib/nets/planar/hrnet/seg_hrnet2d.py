...
Seg2d_HRNetv2_W32 (modified ~20.5M params)
Seg2d_HRNetv2_W48 (modified ~65.7M params)
"""
...
class HighResolutionNet(BaseModel):
    ...
    def __init__(self, config, in_channels, num_classes, **kwargs):
        ...
        # stem net
        self.conv1 = nn.Conv2d(self.in_channels, 64, kernel_size=3, stride=2, 
                               padding=1, bias=False)
        ...
        self.last_layer = nn.Sequential(
            nn.Conv2d(
                ...
                padding=1,
                bias=False),
            BatchNorm2d(self.W, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(self.W, self.W, 4, stride=2, padding=1, 
                               bias=False),
            BatchNorm2d(self.W, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(self.W, self.W, 4, stride=2, padding=1, 
        ...
        print(f'💠 HRNetSeg_2d model initiated with n_classes={num_classes}, '
    ...
    def init_weights(self, pretrained=''):
        ...
        if os.path.isfile(pretrained):
            print(f'  [HRNet-Seg2d] Loading pretrained model {pretrained}..')
            ...
            print(f'[HRNet-Seg2d] Given pretrain file "{pretrained}" is bad.')