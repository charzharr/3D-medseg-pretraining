...
### Common File Functions: Checking, Listing ###
...
def is_image(filepath, exts=None, checkfile=False):
    ...
    if exts:
        if isinstance(exts, str):
            ...
            raise ValueError(f"exts ({exts}) must be a str or a list of strs.")
        assert sum(list(map(lambda x: x[0]=='.', exts))) == len(exts)
    else:
        exts = IMAGE_EXTS
    ext = get_ext(filepath, dot=True)
...
def list_files(path, subnames=[], hidden=False, sort=True, fullpath=False):
    ...
    if fullpath:
        ret_list = [os.path.join(path, fn) for fn in ret_list]
    return ret_list
