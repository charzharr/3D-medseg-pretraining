...
Defines the API regarding common image manipulation, visualization, etc.
Deals with backend representations of images instead of visualization.

Standardized Assumptions:
    Image Size & Channel Configuration
        Colored:     HxWx3 (np)  - [0,255], uint8
                     3xHxW (ten) - [0,255] or [0,1], def: float
        Gray/Binary: HxWx1 or HxW (np)  - [0,255], uint8
                     1xHxW or HxW (ten) - 
    


Common Parameters:
    color or IMAGE_COLORS = color formats for images (e.g. grayscale, rgb, bgr)

Domain and Conversion Notes:
    cv2.imread -> np.array (H,W,Crgb) or (Row,Col,Channelrgb)
    PIL.Image (W,H)
    tensor (C, .. ) [0,1]

	PIL -> np (W,H,C)   pilimg = Image,open(fn); npimg = np.array(pilimg)
	np  -> PIL          pil_im = Image.fromarray(npimg)

    Grayscale Images are assumed be in (H,W) or (H,W,C) format.
        (B,C,H,W) if in stacked tensor.
"""
