...
def conv3x3x3(in_planes, out_planes, kernel_size=(3,3,3), stride=(1,1,1), 
        padding=1, dilation=1, bias=False, weight_std=False):
    "3x3x3 convolution with padding"
    if weight_std:
        return Conv3d(in_planes, out_planes, kernel_size=kernel_size, 
...
class NoBottleneck(nn.Module):
    def __init__(self, inplanes, planes, stride=1, dilation=1, downsample=None, 
            fist_dilation=1, multi_grid=1, weight_std=False):
        super(NoBottleneck, self).__init__()
        self.weight_std = weight_std
        self.gn1 = nn.GroupNorm(16, inplanes)
        self.conv1 = conv3x3x3(inplanes, planes, kernel_size=(3, 3, 3), 
        ...
        self.conv2 = conv3x3x3(planes, planes, kernel_size=(3, 3, 3), stride=1, 
                               padding=(1,1,1), dilation=dilation * multi_grid, 
...
class unet3D(BaseModel):
    def __init__(self, layers, num_classes=3, weight_std = False):
        ...
        self.fusionConv = nn.Sequential(
            ...
            conv3x3x3(256, 256, kernel_size=(1, 1, 1), padding=(0, 0, 0), 
                      weight_std=self.weight_std)
        )
        ...
        self.final_conv = nn.Sequential(
            OrderedDict([
                ('norm1', nn.GroupNorm(16, 32)),
                ('relu1', nn.ReLU(inplace=in_place)),
                ('conv1', nn.Conv3d(32, num_classes, kernel_size=1))
            ])
        )

        tot_params, tot_tparams = self.param_counts
        print(f'💠 DOD-UNet3D model initiated with n_classes={num_classes}, \n'
              f'   layers={layers}, st_std={weight_std}, \n'
