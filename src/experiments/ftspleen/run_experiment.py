...
def run(rank, cfg, inference_metrics_queue):
    ...
    # ------------------ ##  Experiment Setup  ## ------------------ #
    if rank == 0:
        output.header_one('I. {DS} Finetune Training Components Setup')
...
def get_model(cfg, class_bal_bias=False):
    ...
    weights = data_setup.weights_d['spleen']
    percs = 1 / weights
    ...
    if 'cumednet' in cfg.model.name:
        ...
        model = UNet3plus(in_channels, num_classes, 20, deep_sup=deep_sup)
